...
def add_account(configs, account_name, file_path, user_defined_indexes):
    configs["accounts"][account_name] = {
        "csv_path": file_path,
        "columnIndexes": user_defined_indexes,
        "overrides": {},
    }
...
def update_categories(configs, new_categories):
    ...
    for account_name, account_configs in configs["accounts"].items():
        ...
        configs["accounts"][account_name]["overrides"] = {
            k: v
            for k, v in account_configs["overrides"].items()
            if v in new_categories.keys()
        }
